    ...
    stage('Run Docker Image'){
            ...
            withCredentials([string(credentialsId: 'deploymentserverpwd', variable: 'dpPWD')]) {
                  ...
                  sh "sudo sshpass -p ${dpPWD} scp -r stopscript.sh ubuntu@35.197.135.227:/home/devops" 
                  sh "sudo sshpass -p ${dpPWD} ssh -o StrictHostKeyChecking=no -T ubuntu@35.197.135.227${changingPermission}"
                  sh "sudo sshpass -p ${dpPWD} ssh -o StrictHostKeyChecking=no -T ubuntu@35.197.135.227${scriptRunner}"
                  sh "sudo sshpass -p ${dpPWD} ssh -o StrictHostKeyChecking=no -T ubuntu@35.197.135.227${dockerRun}"
            }
